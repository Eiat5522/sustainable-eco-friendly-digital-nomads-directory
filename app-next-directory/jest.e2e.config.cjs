...
const baseModuleNameMapper =
  base && typeof base === 'object' && base.moduleNameMapper ? base.moduleNameMapper : {};
const baseTransformIgnorePatterns =
  base && typeof base === 'object' && base.transformIgnorePatterns
    ? base.transformIgnorePatterns
    : ['/node_modules/'];
try {
  const shouldLog =
    process.env.JEST_CONFIG_DEBUG === '1' || process.env.DEBUG_JEST_CONFIG === '1' || true;
  if (shouldLog) {
    const _lim = v => JSON.stringify(v, null, 2);
  }
} catch {}
...
module.exports = {
  ...
  testMatch: ['<rootDir>/tests/legacy/**/*.test.ts', '<rootDir>/tests/api/**/*-api.test.ts'],
