...
import { registerStaticModuleMocks } from './jest/moduleMocks';
...
// Async IIFE to allow await in CommonJS-loaded setup file
(async () => { try {
  await registerStaticModuleMocks();

  await jest.unstable_mockModule('broadcast-channel', () => {
  ...
  await jest.unstable_mockModule('@/lib/auth/config', async () => {
    const actual = (await jest.importActual('@/lib/auth/config')) as Record<string, unknown>;
...
try {
  ...
  if (srcAuth) {
    ...
    if (process.env.DEBUG_JEST_SETUP === '1') {
