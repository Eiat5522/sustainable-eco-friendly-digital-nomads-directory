...
test.describe('Discovery network resilience', () => {
  ...
  test('shows retry affordance when the search API fails and recovers on retry', async ({ page }) => {
    ...
    const expectedListing = discoveryListings.find((listing) => listing.slug === 'green-cowork-bangkok')
    if (expectedListing) {
