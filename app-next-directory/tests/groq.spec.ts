...
import type { NearbyVenue, SearchResult, WorkspaceResult } from '@/types/query-types';
import {
  cleanupTestListings,
  createTestListings,
  SAMPLE_LISTINGS,
...
test.describe('Complex GROQ Queries', () => {
  ...
  test('getListingStats handles empty categories gracefully', async () => {
    const stats = await getListingStats();
    const emptyCategories = stats.byCategory.filter(
      (cat: { category: string; count: number }) => cat.count === 0
    );
  ...
  test('getDigitalNomadVenueSummary calculates correct totals', async () => {
    ...
    expect(summary.cafes.total).toBeGreaterThanOrEqual(summary.cafes.laptopFriendly);
  });
});
