// app-next-directory/app/listings/[slug]/ListingContent.tsx
...
type Props = { slug: string; userId?: string };
...
async function fetchReviews(listingSlug: string, userId?: string): Promise<Review[]> {
  try {
    ...
    // Assuming getCollection and its underlying fetch mechanism supports revalidate
    // or is handled by a separate caching layer (e.g., MongoDB not directly in Next.js cache)
...
export default async function ListingContent({ slug, userId }: { slug: string; userId?: string }) {
  if (isE2ETest) {
