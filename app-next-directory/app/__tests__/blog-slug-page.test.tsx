...
describe('BlogPostPage', () => {
  it('renders post content, placeholder hero, and comments from DTO response', async () => {
    ...
    const element = await pageModule.default({ params: generateAsyncValue({ slug: 'sustainable-journey' }) });
    render(element);
  ...
  it('supports legacy post payloads with explicit comments', async () => {
    ...
    const element = await pageModule.default({ params: generateAsyncValue({ slug: 'legacy-post' }) });
    render(element);
  ...
  it('handles minimal fallback payloads lacking DTO wrappers', async () => {
    ...
    try {
      ...
      const element = await pageModule.default({ params: generateAsyncValue({ slug: 'fallback-format' }) });
      render(element);
  ...
  it('invokes notFound when API returns 404', async () => {
    ...
    await expect(pageModule.default({ params: generateAsyncValue({ slug: 'missing-post' }) })).rejects.toThrow(
      'NEXT_NOT_FOUND'
    );
...
describe('BlogPostPage.generateMetadata', () => {
  it('returns metadata derived from DTO response with relative image', async () => {
    ...
    const metadata = await pageModule.generateMetadata({ params: generateAsyncValue({ slug: 'meta-post' }) });
  ...
  it('returns not found metadata when API returns 404', async () => {
    ...
    const metadata = await pageModule.generateMetadata({ params: generateAsyncValue({ slug: 'missing-post' }) });
  ...
  it('falls back to generic metadata when the fetch fails', async () => {
    ...
    const metadata = await pageModule.generateMetadata({ params: generateAsyncValue({ slug: 'meta-error' }) });
  ...
  it('returns summary card metadata when image and excerpt are missing', async () => {
    ...
    const metadata = await pageModule.generateMetadata({ params: generateAsyncValue({ slug: 'meta-fallback' }) });
