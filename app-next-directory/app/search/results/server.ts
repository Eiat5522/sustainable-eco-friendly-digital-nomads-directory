...
export async function fetchSearchResults(searchParams: SearchParamRecord): Promise<SearchFetchResult> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { ok: false, reason: 'response', status: response.status, statusText: response.statusText }
    }
    ...
    const listings = rawResults.reduce<ListingSummaryDTO[]>((accumulator, item) => {
      try {
        ...
        console.error('Failed to map search result item:', {
          error,
          itemId: typeof item === 'object' && item && '_id' in item ? (item as { _id?: unknown })._id : undefined,
          itemSlug: typeof item === 'object' && item && 'slug' in item ? (item as { slug?: unknown }).slug : undefined,
        })
      }
      return accumulator
    }, [])
    ...
    const totalPages = Math.max(1, Number.isFinite(totalPagesFromResponse) ? Math.trunc(totalPagesFromResponse) : 1)
    const limit = Math.max(1, Number.isFinite(limitFromResponse) ? Math.trunc(limitFromResponse) : 12)

    const total = Number.isFinite(paginationData?.total ?? 0) ? Number(paginationData?.total ?? 0) : listings.length
    const hasMore = Boolean(paginationData?.hasMore ?? page * limit < total)

    const pageSizeOptions = DEFAULT_PAGE_SIZES.includes(limit as (typeof DEFAULT_PAGE_SIZES)[number])
