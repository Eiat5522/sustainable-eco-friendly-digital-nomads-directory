...
describe('Search results page module', () => {
  ...
  it('logs and recovers from unexpected payload shapes', async () => {
    ...
    mockSearchHandler.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ data: { results: { unexpected: true } } }),
        { status: 200 }
      )
    );
    ...
    expect(errorSpy).toHaveBeenCalled();
    expect(errorSpy.mock.calls.some(([message]) => message === 'Unexpected search API payload shape:')).toBe(true);
  ...
  it('builds pagination links and preserves existing search parameters', async () => {
    ...
    expect(prevLink).toHaveAttribute(
      'href',
      expect.stringContaining('tags=wifi')
    );
    ...
    expect(Array.from(pageSizeSelect.options).map((o) => o.value)).toEqual(['12', '24', '48', '96']);
  });
});
