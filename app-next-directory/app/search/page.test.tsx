...
const searchFiltersFormMock = jest.fn((props: { initialParams: SearchParamRecord; resultsPath?: string }) => (
  <div
...
describe('SearchPage', () => {
  ...
  it('renders search results using backend data and forwards params to filters', async () => {
    ...
    const searchParams = Promise.resolve({ q: 'eco hubs', destination: ['bangkok'], limit: '24' } as SearchParamRecord)
  ...
  it('defaults to empty params when searchParams is undefined', async () => {
    ...
    expect(searchFiltersFormMock).toHaveBeenCalledWith(expect.objectContaining({ initialParams: {}, resultsPath: '/search' }))
  })
  ...
  it('handles backend errors by showing a retry state', async () => {
    ...
    const page = await SearchPage({ searchParams: Promise.resolve({ retry: '2' } as SearchParamRecord) })
    render(page)
