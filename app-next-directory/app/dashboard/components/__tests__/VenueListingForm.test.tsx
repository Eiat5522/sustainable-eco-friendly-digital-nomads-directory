...
jest.mock('@/components/ui/select', () => {
  ...
  const SelectItemComponent = ({ value, children }: any) => <option value={value}>{children}</option>
...
describe('VenueListingForm', () => {
    ...
    const fetchMock = jest.fn(async (input: RequestInfo | URL) => {
      ...
      if (url === '/api/amenities') {
        return {
          json: async () => ({
            amenities: [
              { _id: 'amenity-1', name: 'Amenity Valid' },
              null,
              { _id: 'amenity-2' },
            ],
          }),
  ...
  it('uploads image assets and forwards normalized payload to onSave', async () => {
    ...
    const { container } = render(
      <VenueListingForm
        listing={{ name: 'Existing Venue', city: 'city-1', type: 'coworking' } satisfies Partial<ListingFormValues> }
        onSave={onSave}
      />,
    )
    ...
    expect(payload.galleryImages).toEqual([
      { _type: 'image', _key: 'upload-gallery-1', asset: { _type: 'reference', _ref: 'upload-gallery-1' } },
      { _type: 'image', _key: 'upload-gallery-2', asset: { _type: 'reference', _ref: 'upload-gallery-2' } },
    ])
  ...
  it('submits without image selections and keeps image fields undefined', async () => {
    ...
    const { container } = render(
      <VenueListingForm
        listing={{ name: 'No Images Venue', city: 'city-1', type: 'cafe' } satisfies Partial<ListingFormValues> }
        onSave={onSave}
      />,
    )
    ...
    const [primaryInput, galleryInput] = container.querySelectorAll('input[type="file"]') as NodeListOf<HTMLInputElement>
    ...
    const uploadCalls = fetchMock.mock.calls.filter(([request]) =>
      (typeof request === 'string' ? request : request.toString()) === '/api/upload',
    )
  ...
  it('renders configuration sections for each listing type', async () => {
    ...
    const planTypeInput = container.querySelector('input[name="coworkingDetails.pricingPlans.0.type"]') as HTMLInputElement
    const planPriceInput = container.querySelector('input[name="coworkingDetails.pricingPlans.0.price"]') as HTMLInputElement
    const planPeriodInput = container.querySelector('input[name="coworkingDetails.pricingPlans.0.period"]') as HTMLInputElement
    ...
    const accommodationTypeSelect = accommodationContainer.querySelector('select') as HTMLSelectElement
    fireEvent.change(accommodationTypeSelect, { target: { value: 'hotel' } })
    const minStayInput = accommodationContainer
      .querySelector('input[name="accommodationDetails.minimumStay"]') as HTMLInputElement
    ...
    const zeroWasteCheckbox = getCheckboxWithin(activitiesSection.closest('div') as HTMLElement, 'Zero Waste')
    fireEvent.click(zeroWasteCheckbox, { target: { checked: true } })
    fireEvent.click(zeroWasteCheckbox, { target: { checked: false } })
    const englishCheckbox = getCheckboxWithin(activitiesSection.closest('div') as HTMLElement, 'English')
  ...
  it('indicates saving state on the submit button', async () => {
    ...
    render(
      <VenueListingForm
        saving
        listing={{ name: 'Saving Venue', city: 'city-1', type: 'cafe' } satisfies Partial<ListingFormValues> }
      />, 
    )
  ...
  it('logs an error when an upload request fails', async () => {
    ...
    const { container } = render(
      <VenueListingForm
        listing={{ name: 'Venue Error', city: 'city-1', type: 'coworking' } satisfies Partial<ListingFormValues> }
        onSave={onSave}
      />,
    )
    ...
    const [primaryInput] = container.querySelectorAll('input[type="file"]') as NodeListOf<HTMLInputElement>
