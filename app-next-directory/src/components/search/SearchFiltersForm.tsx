...
function toStringArray(value: string | string[] | undefined): string[] {
  ...
  return Array.from(
    new Set(arr.map(entry => String(entry).trim()).filter(entry => entry.length > 0))
  );
}
      ...
      try {
        const [citiesRes, categoriesRes, amenitiesRes] = await Promise.all([
          fetch('/api/cities', { signal })
            .then(response =>
              response.ok
                ? (response.json() as Promise<CityResponse>)
          ...
          fetch('/api/categories', { signal })
            .then(response =>
              response.ok
                ? (response.json() as Promise<CategoryResponse>)
                : Promise.reject(response.statusText)
            )
          ...
          fetch('/api/amenities', { signal })
            .then(response =>
              response.ok
                ? (response.json() as Promise<AmenityResponse>)
                : Promise.reject(response.statusText)
            )
