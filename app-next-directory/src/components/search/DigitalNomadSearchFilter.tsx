...
import { useFilters, type FilterDefinition } from '@/hooks/useFilters'
...
export function DigitalNomadSearchFilter({ definitions, initialFilters, onChange, title = 'Filters' }: DigitalNomadSearchFilterProps) {
              ...
              {group.options.map((opt) => {
                ...
                return (
                  <label key={opt.id} className="inline-flex items-center gap-2">
                    ...
                    />
                    <span className="body-md">{opt.label}{typeof opt.count === 'number' ? ` (${opt.count})` : ''}</span>
                  </label>
                )
              })}
            </div>
