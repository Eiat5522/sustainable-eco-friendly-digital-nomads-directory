...
jest.mock('lucide-react', () => ({
  ...
  Heart: function MockHeart({ size, className }: any) {
    return <span data-testid="heart-icon" data-size={size} className={className}>❤️</span>;
  },
}));
...
jest.mock('@/components/ui/neo-card', () => ({
  ...
  NeoCardTitle: function MockNeoCardTitle({ children, className }: any) {
    return <h1 className={className} data-testid="neo-card-title">{children}</h1>;
  },
}));
...
describe('HeroSection', () => {
  ...
  it('renders listing information correctly', () => {
    ...
    expect(screen.getByText('Eco-friendly coffee shop with organic beans and solar power')).toBeInTheDocument();
  ...
  it('handles listing without short description', () => {
    ...
    expect(screen.queryByText('Eco-friendly coffee shop with organic beans and solar power')).not.toBeInTheDocument();
  });
  ...
  describe('Favorite functionality', () => {
    ...
    it('works when onToggleFavorite is not provided', () => {
      render(
        <HeroSection 
          listing={mockListing}
          isFavorited={false}
      ...
      // Should not throw error when clicked without onToggleFavorite
    });
  ...
  describe('Component structure and styling', () => {
    ...
    it('renders with correct hero image container styles', () => {
      ...
      // Check if the hero image container has the correct classes
      const heroContainer = screen.getByTestId('next-image').parentElement;
      expect(heroContainer).toHaveClass('relative', 'h-64', 'md:h-80', 'mb-6', 'overflow-hidden', 'rounded-lg');
    });
  });
});