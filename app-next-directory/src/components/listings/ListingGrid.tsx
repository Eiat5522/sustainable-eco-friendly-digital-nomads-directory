...
interface ListingGridProps {
  readonly listings: ReadonlyArray<AppListingCard>;
}
...
export function ListingGrid(
  { listings }: Readonly<ListingGridProps>
) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12 p-4 md:p-6">
      {listings.map((listing, index) => {
        const resolvedSlug = resolveSlug(listing.slug);
        const href = resolvedSlug
          ? { pathname: '/listings/[slug]', query: { slug: encodeURIComponent(resolvedSlug) } }
          : '/listings/';

        // Determine badge class without nested ternary (Sonar rule typescript:S3358)
        const typeBadgeClass = (() => {
          switch (listing.type) {
            case 'coworking':
              return 'bg-category-coworking text-white';
            case 'cafe':
              return 'bg-category-cafe text-white';
            default:
              return 'bg-category-accommodation text-white';
          }
        })();
        return (
        <div
