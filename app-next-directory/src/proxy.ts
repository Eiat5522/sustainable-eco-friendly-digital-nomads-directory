...
if (typeof module !== 'undefined' && module.exports) {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  module.exports = { proxy: require('@/lib/auth').auth, config, createMiddleware };
}

// Provide a top-level `proxy` export for Next.js runtime.
import { NextResponse as NextResponseReal } from 'next/server';

const NextResponseFactory = {
  next: NextResponseReal.next.bind(NextResponseReal),
  redirect: NextResponseReal.redirect.bind(NextResponseReal),
  json: NextResponseReal.json.bind(NextResponseReal),
};

async function noopGetToken(): Promise<null> {
  return null;
}
