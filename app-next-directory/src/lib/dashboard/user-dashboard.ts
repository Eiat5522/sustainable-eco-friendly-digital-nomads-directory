...
export async function getUserDashboardData(
  ...
  const cacheKey = `user-dashboard:${sessionUser.id}:${months}`;

  const cachedData = await cacheService.get<DashboardPayload>(cacheKey);
  if (cachedData) {
    return cachedData;
  }

  const freshData = await (async (): Promise<DashboardPayload | null> => {
  ...
  if (freshData) {
    await cacheService.set(cacheKey, freshData, config.redis.dashboardTTL);
  }
