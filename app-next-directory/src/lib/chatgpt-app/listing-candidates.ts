...
import { ListingCandidateSchema, type ListingCandidate, type ListingType } from './workday-schemas';

const COMPONENT = 'chatgpt-app/listing-candidates';
...
const mapRawListing = (raw: RawListingCandidate): ListingCandidate | null => {
...
const mapRawListings = (raw: unknown): ListingCandidate[] => {
  if (!Array.isArray(raw)) return [];
  return raw.flatMap((entry): ListingCandidate[] => {
    if (!entry || typeof entry !== 'object') return [];
    const mapped = mapRawListing(entry as RawListingCandidate);
...
export async function fetchListingCandidates({
  ...
  try {
    const raw = await client.fetch<unknown>(LISTING_CANDIDATES_QUERY, {
      city: normalizeText(city),
      limit,
    });
    return mapRawListings(raw);
  } catch (error) {
    structuredLogger.error('Failed to fetch ChatGPT listing candidates', error, {
      component: COMPONENT,
      city,
    });
    return [];
...
export async function searchListingReferences(query: string): Promise<ReferenceResult[]> {
  try {
    const pattern = `*${normalizeText(query)}*`;
    const raw = await client.fetch<unknown>(LISTING_REFERENCES_QUERY, { pattern });
    return mapRawListings(raw).map(candidate => ({
      id: candidate.id,
      title: candidate.name,
      url: candidate.canonicalUrl,
    }));
  } catch (error) {
    structuredLogger.error('Failed to search ChatGPT listing references', error, {
      component: COMPONENT,
      query,
    });
    return [];
...
export async function fetchListingCandidate(identifier: string): Promise<ListingCandidate | null> {
  try {
    const raw = await client.fetch<unknown>(LISTING_BY_ID_OR_SLUG_QUERY, { identifier });
    if (!raw || typeof raw !== 'object') return null;
    return mapRawListing(raw as RawListingCandidate);
  } catch (error) {
    structuredLogger.error('Failed to fetch ChatGPT listing candidate', error, {
      component: COMPONENT,
      identifier,
    });
    return null;
