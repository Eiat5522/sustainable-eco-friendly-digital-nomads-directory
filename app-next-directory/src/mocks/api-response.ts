...
type ApiResponseHandlerMocks = {
  ...
  paginated: jest.MockedFunction<
    (items: unknown[], page?: number, limit?: number) => PaginatedResponse<unknown>
  >;
};

export const ApiResponseHandler: ApiResponseHandlerMocks = {
  success: jest.fn<(data: unknown, message?: string) => SuccessResponse<unknown>>(
    (data, message) => {
