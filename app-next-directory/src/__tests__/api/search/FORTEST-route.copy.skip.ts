...
// Build expected GROQ slice range (end is exclusive)
const groqSlice = (page: number, limit: number) => {
  const start = Math.max(0, (Math.max(1, page) - 1) * Math.max(1, limit));
  return `[${start}...${start + Math.max(1, limit)}]`;
};
...
describe('Search API Route', () => {
  ...
  describe('GET /api/search', () => {
    it('should handle basic search query', async () => {
      ...
      expect(mockApiResponseHandler.success).toHaveBeenCalledWith(expect.objectContaining({
        results: mockResults,
        ...
        filters: expect.objectContaining({ query: 'coworking' }),
      }));
    });
    ...
    it('should handle search with multiple filters', async () => {
      ...
      expect(firstCall).toContain('[6...12]');
    });
    ...
    it('should handle pagination correctly', async () => {
      ...
      const firstCall: string = mockClient.fetch.mock.calls[0][0];
      expect(firstCall).toContain('[10...15]');
    });
    ...
    it('should handle invalid pagination parameters', async () => {
      ...
      expect(mockApiResponseHandler.success).toHaveBeenCalledWith(expect.objectContaining({
        results: mockResults,
        pagination: expect.objectContaining({ page: 1, limit: 1, total: 0, totalPages: 0, hasMore: false }),
      }));
    });
    ...
    it('should handle very large page numbers', async () => {
      ...
      const firstCall: string = mockClient.fetch.mock.calls[0][0];
      expect(firstCall).toContain('[999800...999900]');
    });
  });
  ...
  describe('POST /api/search', () => {
    ...
    it('should use correct URL construction in POST', async () => {
      ...
      expect(mockClient.fetch).toHaveBeenCalledTimes(2);
      const firstCall: string = mockClient.fetch.mock.calls[0][0];
      expect(firstCall).toContain('test query');
      expect(firstCall).toContain('[16...24]'); // page=3, limit=8
    });
  });
