 ...
 * Exported for testing and used by the background interval.
 */
...
// Use undefined initially to signal it hasn't been attempted yet
  ...
  if (redisClient) {
    ...
    return async (request: Request): Promise<RateLimitResult> => {
      try {
        ...
        const key = keyGenerator ? keyGenerator(request) : extractClientIp(request);
