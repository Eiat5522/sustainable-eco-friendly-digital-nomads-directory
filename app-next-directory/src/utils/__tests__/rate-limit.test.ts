...
describe('rate-limit', () => {
  ...
  describe('rateLimit', () => {
    ...
    it('should block requests when limit is exceeded', () => {
      ...
      limiter(request); // First request
      limiter(request); // Second request
      
      const result = limiter(request); // Third request should be blocked
    ...
    it('should reset count after window expires', () => {
      ...
      const newResult = limiter(request);
    ...
    it('should handle multiple requests at the exact limit', () => {
  ...
  describe('rateLimiters', () => {
    ...
    it('contactForm limiter should enforce 5 requests limit', () => {
      const request = new Request('http://localhost', {
        headers: { 'x-forwarded-for': '127.0.0.1' },
      });
      ...
      for (let i = 0; i < 5; i++) {
        const result = rateLimiters.contactForm(request);
        expect(result.success).toBe(true);
      }
    ...
    it('apiGeneral limiter should enforce 100 requests limit', () => {
      const request = new Request('http://localhost', {
        headers: { 'x-forwarded-for': '127.0.0.2' },
      });
      ...
      for (let i = 0; i < 100; i++) {
        const result = rateLimiters.apiGeneral(request);
        expect(result.success).toBe(true);
      }
    ...
    it('search limiter should enforce 50 requests limit', () => {
      const request = new Request('http://localhost', {
      ...
      for (let i = 0; i < 50; i++) {
        const result = rateLimiters.search(request);
        expect(result.success).toBe(true);
      }
  ...
  describe('rateLimitStore', () => {
    ...
    it('should store rate limit information', () => {
      const limiter = rateLimit({ max: 1, windowMs: 1000 });
      const request = new Request('http://localhost', {
        headers: { 'x-forwarded-for': '127.0.0.1' },
      });
