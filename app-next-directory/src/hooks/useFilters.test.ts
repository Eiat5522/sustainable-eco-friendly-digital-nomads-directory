...
describe('useFilters', () => {
  ...
  it('should initialize with initial filters', () => {
    ...
    const { result } = renderHook(() => useFilters({ definitions, initialFilters }));
  ...
  it('should call onFilterChange when filters change', () => {
    ...
    const { result } = renderHook(() => useFilters({ definitions, onFilterChange }));
  ...
  it('should return active filters for a group', () => {
    ...
    const { result } = renderHook(() => useFilters({ definitions, initialFilters }));
    expect(result.current.getActiveFiltersForGroup('amenities')).toEqual(['wifi', 'coffee']);
