...
describe('useSearch', () => {
  ...
  it('performs debounced searches and fetches suggestions', async () => {
    ...
    const { result } = renderHook(() =>
      useSearch({ debounceMs: 20, initialFilters: { query: '', ecoTags: [], hasDigitalNomadFeatures: false } })
    )
  ...
  it('surfaces API failures as error states', async () => {
    mockFetch
      .mockResolvedValueOnce(
        createResponse({ results: [], pagination: { total: 0, page: 1, totalPages: 0, hasMore: false }, error: null })
      )
      .mockResolvedValueOnce(
        createResponse(
          {},
  ...
  it('normalises malformed responses and resets filters and sort', async () => {
    mockFetch.mockResolvedValue(createResponse({ results: 'unexpected', pagination: undefined, error: null }))
