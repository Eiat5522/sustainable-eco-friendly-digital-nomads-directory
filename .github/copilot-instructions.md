...
- **Task Management**: Use the task management system to track your progress. Mark tasks as complete when finished, and update the status of subtasks accordingly.
- **Subtask Completion**: You must (Without fail) update tasks and subtasks status once you have completely finished the corresponding tasks or subtasks.
...
- **Change Directory Best Practices:**
  ...
  - Examples:
    ...
    Set-Location -Path "d:\Eiat_Folder\MyProjects\MyOtherProjects\sustainable-eco-friendly-digital-nomads-directory\app-scaffold"
...
- **Endpoint Structure:** REST-like endpoints under `/src/app/api/*`
  (📌 _SustainableDigitalNomadsDirectory also uses `app-scaffold/src/app` for Next.js API Routes_)
...
Be attentive to any new information in these categories:
