...
## GitHub Copilot Instructions for Sustainable Eco-Friendly Digital Nomads Directory
...
- **Task Management**: Follow the Workstream Documentation {"./memory-bank/parallel_workstreams.md"} to track latest Project's status, Before a Task is to be CONSIDERDED as Completed, Copilot must reassure that the task is actually completed. If the Tasks is UX/UI related. An #MCP Tool #Playwrite or #BrowserTool must be use for confirmation. Only then Copilot must Update Task Status to Complete.
...
## Mandatory: Sanity TypeGen Usage

- After any change to Sanity schemas or GROQ queries, you must run Sanity TypeGen to regenerate TypeScript types.
- Command: `pnpm run codegen:sanity`
- Generated types are used for:
  - Schema validation
  - DTO alignment in `app-next-directory/src/types/appView.ts`
  - Type-safe GROQ query results
- All code interacting with Sanity data must use these generated types for strict type safety.
- PRs will not be accepted unless types are up-to-date and match the latest schema and queries.

Example workflow:
1. Update schema in `sanity/schemas/`
2. Run `pnpm run codegen:sanity`
3. Use generated types in DTOs and API logic
...
When navigating the project directory, please use PowerShell 7 and follow the best practices and examples provided in the "📑 **Directory Navigation with PowerShell 7**"
