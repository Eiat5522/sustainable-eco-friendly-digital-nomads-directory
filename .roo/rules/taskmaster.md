...
*   **Description:** `Parse a Product Requirements Document, PRD, or text file with Taskmaster to automatically generate an initial set of tasks in tasks.json. Incorporates project context from the memory-bank folder when available.`
*   **Key Parameters/Options:**
    ...
    *   `memoryBank`: `Path to the memory-bank folder containing additional context files. Defaults to '.clinerules/memory-bank'.` (CLI: `-m, --memory-bank <path>`)
    *   `includeMemory`: `Whether to include memory bank information in task generation. Defaults to true.` (CLI: `--include-memory`)
*   **Usage:** Useful for bootstrapping a project from an existing requirements document while incorporating project memory.
*   **Notes:** Task Master will strictly adhere to any specific requirements mentioned in the PRD, such as libraries, database schemas, frameworks, tech stacks, etc., while filling in any gaps where the PRD isn't fully specified. When memory bank files are available, those will be incorporated to ensure task generation includes architectural decisions, technical constraints, and established patterns. Tasks are designed to provide the most direct implementation path while avoiding over-engineering.
*   **Important:** This MCP tool makes AI calls and can take up to a minute to complete. Please inform users to hang tight while the operation is in progress. If the user does not have a PRD, suggest discussing their idea and then use the example PRD in `scripts/example_prd.txt` as a template for creating the PRD based on their idea, for use with `parse-prd`. For memory-bank files, recommend creating entries in `.clinerules/memory-bank` to document key project decisions and context.
