...
export default {
  ...
  fields: [
    ...
    { name: 'pricingPlans', type: 'array', of: [{ type: 'coworkingPricingPlan' }], validation: Rule => Rule.required().min(1) },
    ...
    // Internet speed (required)
    {
      ...
      type: 'object',
      fields: [
        { name: 'download', title: 'Download Speed (Mbps)', type: 'number', validation: Rule => Rule.required().min(1) },
        { name: 'upload', title: 'Upload Speed (Mbps)', type: 'number', validation: Rule => Rule.required().min(1) },
        { name: 'lastTested', title: 'Last Tested', type: 'datetime', validation: Rule => Rule.required() }
      ],
