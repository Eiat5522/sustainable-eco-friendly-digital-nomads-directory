  ...
  return (
    <section className="mt-10">
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {related.map((listing) => (
          <li key={listing.id} className="p-4 bg-stone-50 dark:bg-slate-800 rounded shadow">
            <Link href={`/listings/${listing.id || listing.slug}`} className="text-lg font-semibold text-emerald-700 dark:text-emerald-300 hover:underline">
              {listing.name}
            </Link>
